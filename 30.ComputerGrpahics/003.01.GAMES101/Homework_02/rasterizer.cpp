...
static bool insideTriangle(float x, float y, const Vector3f* _v) {
	...
	float a = ab.cross(ap).z();
	float b = bc.cross(bp).z();
	float c = ca.cross(cp).z();
	...
	for (int x = x_min; x <= x_max; ++x) {
		for (int y = y_min; y <= y_max; ++y) {
			...
			for (int i = 0; i < offsets.size(); ++i) {
				float sample_x = x + offsets[i].x();
				float sample_y = y + offsets[i].y();
				if (insideTriangle(sample_x, sample_y, t.v)) {
					...
					if (z_interpolated < ssaa_depth_buf[get_index(x, y)][i]) {
						...
						ssaa_frame_buf[get_index(x, y)][i] = t.getColor();
					}
				}
			}
			...
			for (int i = 0; i < offsets.size(); ++i) {
				color += ssaa_frame_buf[get_index(x, y)][i];
			}
			frame_buf[get_index(x, y)] = color / offsets.size();
#else
...
void rst::rasterizer::clear(rst::Buffers buff) {
	...
	for (int i = 0; i < width * height; ++i) {
		ssaa_depth_buf[i].fill(std::numeric_limits<float>::infinity());
		ssaa_frame_buf[i].fill(Eigen::Vector3f {0, 0, 0});
	}
#endif
}
